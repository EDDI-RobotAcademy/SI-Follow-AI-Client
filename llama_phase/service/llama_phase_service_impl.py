from llama_phase.repository.llama_phase_repository_impl import LlamaPhaseRepositoryImpl
from llama_phase.service.llama_phase_service import LlamaPhaseService


class LlamaPhaseServiceImpl(LlamaPhaseService):
    __instance = None

    def __new__(cls):
        if cls.__instance is None:
            cls.__instance = super().__new__(cls)
            cls.__instance.__phaseRepository = LlamaPhaseRepositoryImpl.getInstance()
    ...
    def get_current_phase(self, *args, **kwargs):
        ...
        return {"user_token": args[0], "project_name": args[1], "phase": current_phase}

    def get_backlogs(self, *args, **kwargs):
        backlog = self.__phaseRepository.get_backlogs(*args, **kwargs)
        return {"user_token": args[0], "project_name": args[1], "backlog": backlog}
